...
func Initialise(cfg config.YoutubeConfig) (YouTubeService, error) {
	client := &http.Client{
...
func (w *YouTubeService) FetchYoutubeVideos()([]models.YoutubeVideo, error) {
	call := w.service.Search.List([]string{"snippet"}).
                Q(w.query).
                MaxResults(int64(w.maxResults)).
				RelevanceLanguage(w.relevanceLanguage)
